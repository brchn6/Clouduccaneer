...
def fetch_many(urls: List[str], out_template: str, audio_fmt="mp3", quality="0",
    ...
    if dry:
        for u in urls: print("[DRY] would fetch:", u)
        return 0
    rc = 0
    for u in urls:
        rc = fetch(u, out_template, audio_fmt, quality, embed, add_meta, write_thumb, convert_jpg, parse_meta) or rc
    return rc
...
def sc_search_urls(query: str, kind: str = "tracks", max_results: int = 20) -> List[str]:
    ...
    return list(islice(print_lines(["yt-dlp", "--flat-playlist", "--print", "%(url)s", q]), n))

def sc_search_url_title_pairs(query: str, kind: str = "tracks", max_results: int = 20) -> List[tuple[str,str]]:
    ...
    for line in print_lines(["yt-dlp", "--flat-playlist", "--print", "%(url)s\t%(uploader_id|uploader)s", q]):
        if "\t" in line:
