...
def fetch(
    ...
    limit_seconds: int = typer.Option(
        None, "--max-seconds", help="Skip tracks longer than this"
    ),
...
@app.command()
def rename(
    folder: Path = typer.Argument(..., help="Folder to clean"),
...
@app.command()
def dedupe(
    root: Path = typer.Argument(..., help="Kill *.1.mp3 style dupes"),
    apply: bool = typer.Option(False, "--apply"),
):
    ...
    victims = list(
        p
        for p in root.rglob("*")
        if p.is_file() and p.suffix.lower() == ".mp3" and p.stem.endswith(".1")
    )
    for v in victims:
...
def search(
    ...
    kind: str = typer.Option(
        "tracks", "--kind", help="'tracks' (default), 'sets', or 'users'"
    ),
    ...
    if cluster:
        ...
        print(
            f"Found {sum(len(v) for v in buckets.values())} result(s) in {len(buckets)} bucket(s)."
        )
        for uploader, urls in buckets.items():
            ...
            if dry:
                ...
                out_tmpl = str(
                    base
                    / uploader
                    / cfg.get("out_template", "%(title)s - %(artist|uploader)s.%(ext)s")
                )
                ytwrap.fetch_many(
                    urls, out_tmpl, max_seconds=max_seconds, write_thumb=False
                )
        return
    ...
    out_tmpl = str(
        base / cfg.get("out_template", "%(title)s - %(artist|uploader)s.%(ext)s")
    )
    ytwrap.fetch_many(
        urls, out_tmpl, max_seconds=max_seconds, dry=dry, write_thumb=False
    )
...
def fetch_user(
    user_url: str,
    kind: str = typer.Option(
        "uploads", "--kind", help="'uploads'|'reposts'|'likes'|'sets'"
    ),
    dest: Path = typer.Option(None, "--dest"),
    max_seconds: Optional[int] = typer.Option(None, "--max-seconds"),
    limit: Optional[int] = typer.Option(
        None, "--limit", help="Only take the most recent N items"
    ),
    ...
    if kind not in path_map:
        print("Invalid --kind. Use uploads|reposts|likes|sets")
        raise typer.Exit(2)
...
@app.command()
def clean(
    folder: Path = typer.Argument(..., help="Folder to sweep"),
    images: bool = typer.Option(
        True, "--images/--no-images", help="Remove leftover image files"
    ),
    parts: bool = typer.Option(
        True, "--parts/--no-parts", help="Remove temp/partial files"
    ),
...
def fetch_spotify(
    ...
    quality: str = typer.Option(
        "320k", "--quality", help="Audio quality (320k, 256k, 192k, 128k)"
    ),
    format: str = typer.Option(
        "mp3", "--format", help="Audio format (mp3, flac, ogg, m4a)"
    ),
    ...
    cfg = load_config()
    base = Path(
        dest or cfg.get("spotify", {}).get("download_dir", "~/Download/spotify")
    ).expanduser()
    base.mkdir(parents=True, exist_ok=True)
...
def search_spotify(
    ...
    type_filter: str = typer.Option(
        "track", "--type", help="Search type: track, album, playlist, artist"
    ),
    dest: Path = typer.Option(None, "--dest", help="Destination directory"),
...
def summarize(
    conversation: str = typer.Argument(
        ..., help="Conversation input as JSON string or file path"
    ),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Output file for summary"
    ),
):
    """
    ...
    if not isinstance(conversation_data, list):
        print(
            "It seems that the conversation you intended to provide is "
            "incomplete. Please provide the full series of user/assistant "
            "message pairs so I can create a detailed summary for you."
        )
        raise typer.Exit(code=1)
...
def _create_conversation_summary(conversation_data: List[Dict]) -> str:
    ...
    summary_lines.extend(
        [
            "",
            "## Conversation Flow:",
            f"The conversation began with user asking about "
            f"{user_messages[0][:50] + '...' if len(user_messages[0]) > 50 else user_messages[0]}",
        ]
    )
    ...
    if len(user_messages) > 1:
        summary_lines.append(
            f"The conversation continued with {len(user_messages) - 1} additional user message(s)."
        )
...
@app.command()
def bpm(
    target: Path = typer.Argument(..., help="Audio file or directory to analyze"),
    ...
    export_filename: bool = typer.Option(
        False,
        "--export-filename",
        help="Add BPM to filename (e.g., song [128 BPM].mp3)",
    ),
    export_tags: bool = typer.Option(
        False, "--export-tags", help="Add BPM to audio file metadata tags"
    ),
