...
class TestLoadConfig:
    ...
    def test_load_config_from_file(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch('pathlib.Path.exists', return_value=True):
                with patch('pathlib.Path.open', mock_open(read_data=yaml.dump(test_config))):
                    config = load_config()
    ...
    def test_load_config_invalid_yaml(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch('pathlib.Path.exists', return_value=True):
                with patch('pathlib.Path.open', mock_open(read_data="invalid: yaml: content:")):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch('pathlib.Path.exists', return_value=True):
                with patch('pathlib.Path.open', mock_open(read_data=yaml.dump(test_config))):
                    ...
                    assert "keep_track" not in config["rename"]  # Not preserved due to shallow update
                    assert config["spotify"]["quality"] == "128k"  # Overridden
                    assert "format" not in config["spotify"]  # Not preserved due to shallow update    def test_load_config_file_permissions_error(self):
    ...
    def test_config_structure_completeness(self):
        ...
        spotify_keys = ["download_dir", "quality", "format", "lyrics", "playlist_numbering"]
        for key in spotify_keys:
            assert key in config["spotify"], f"Missing required spotify config key: {key}"
