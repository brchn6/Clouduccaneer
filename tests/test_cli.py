...
class TestFetchCommand:
    ...
    def test_fetch_dry_run(self, mock_fetch, mock_load_config, runner, mock_config):
        ...
        result = runner.invoke(
            app, ["fetch", "https://soundcloud.com/test/track", "--dry"]
        )
    ...
    @patch("cb.cli.ytwrap.fetch")
    def test_fetch_with_duration_limit(
        self,
        mock_fetch,
        mock_duration_map,
        mock_print_lines,
        ...
        result = runner.invoke(
            app,
            ["fetch", "https://soundcloud.com/test/playlist", "--max-seconds", "200"],
        )
    ...
    @patch("cb.cli.load_config")
    def test_fetch_with_custom_dest(
        self, mock_load_config, runner, mock_config, temp_dir
    ):
        ...
        with patch("cb.cli.ytwrap.fetch") as mock_fetch:
            result = runner.invoke(
                app,
                ["fetch", "https://soundcloud.com/test/track", "--dest", str(temp_dir)],
            )
...
class TestRenameCommand:
    ...
    def test_rename_no_changes(
        self, mock_plan_renames, mock_load_config, runner, mock_config, temp_dir
    ):
    ...
    def test_rename_with_changes_apply(
        self,
        mock_apply_changes,
        mock_plan_renames,
        mock_load_config,
    ...
    @patch("cb.cli.load_config")
    @patch("cb.cli.plan_renames")
    def test_rename_custom_options(
        self, mock_plan_renames, mock_load_config, runner, mock_config, temp_dir
    ):
        ...
        result = runner.invoke(
            app,
            ["rename", str(temp_dir), "--no-ascii", "--no-keep-track", "--move-covers"],
        )
...
class TestSearchCommand:
    ...
    def test_search_no_results(
        self, mock_sc_search_urls, mock_load_config, runner, mock_config
    ):
    ...
    def test_search_with_clustering(
        self, mock_sc_search_url_title_pairs, mock_load_config, runner, mock_config
    ):
...
class TestClusterUserCommand:
    ...
    @patch("cb.cli.ytwrap.fetch_many")
    def test_cluster_user_basic(
        self, mock_fetch_many, mock_list_flat, mock_load_config, runner, mock_config
    ):
    ...
    @patch("cb.cli.load_config")
    @patch("cb.cli.ytwrap.list_flat")
    def test_cluster_user_dry_run(
        self, mock_list_flat, mock_load_config, runner, mock_config
    ):
        ...
        result = runner.invoke(
            app, ["cluster-user", "https://soundcloud.com/testuser", "--dry"]
        )
...
class TestFetchUserCommand:
    ...
    @patch("cb.cli.load_config")
    @patch("cb.cli.ytwrap.list_flat")
    def test_fetch_user_with_limit(
        self, mock_list_flat, mock_load_config, runner, mock_config
    ):
        ...
        with patch("cb.cli.ytwrap.fetch_many") as mock_fetch_many:
            result = runner.invoke(
                app, ["fetch-user", "https://soundcloud.com/testuser", "--limit", "3"]
            )
